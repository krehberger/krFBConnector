...
Properties {
  ...
  $BuildFolder = "$ProjectRoot\_build\$ModuleName"
...
Task Build {
  ...
  $FunctionsPublic = Get-ChildItem -Path $ProjectRoot\src\public -Recurse -Exclude *.Tests.* -File `
    | ForEach-Object -Process {Get-Content -Path $_.FullName; "`r`n"}
  $FunctionsPrivate = Get-ChildItem -Path $ProjectRoot\src\private -Recurse -Exclude *.Tests.* -File `
    | ForEach-Object -Process {Get-Content -Path $_.FullName; "`r`n"}
  ...
  $Null = Copy-Item   -Path "$ProjectRoot\$ModuleName.psd1" -Destination "$VersionFolder\$ModuleName.psd1" -Force
